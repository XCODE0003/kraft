    ...
    <Layout>
        <main class="flex flex-col py-14 gap-12">
            ...
            <section class="flex container mx-auto flex-col gap-2">
                <Product v-for="product in props.products.data" :product="product" />

                <div v-if="props.products.total > 0" class="flex justify-center gap-5 mt-8">
                    <button @click="goToPage(props.products.current_page - 1)"
                    ...
                    <div class="flex gap-1">
                        <template v-for="page in getVisiblePages()" :key="page">
                            ...
                            <span v-else class="rounded-2xl w-12 h-12 items-center justify-center flex"
                                :class="{ 'bg-black text-white': page === props.products.current_page }"
                                @click="goToPage(page)" style="cursor: pointer">
                                {{ page }}
                            </span>
                        </template>
                    </div>
                    <button @click="goToPage(props.products.current_page + 1)"
                        ...
                        <svg width="7.500000" height="13.333313" viewBox="0 0 7.5 13.3333" fill="none"
                            ...
                            <path id="Icon"
                                ...
                                :fill="props.products.current_page === totalPages ? '#CCCCCC' : '#000000'" />
                        </svg>
                    </button>
                </div>
            </section>
